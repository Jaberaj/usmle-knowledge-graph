...
def main() -> None:
    ...
    for filename, terms in GROUPS.items():
        selected = [entry for entry in entries if any(term in entry["canonical_name"].lower() for term in terms)]
        assigned.update(entry["disease_id"] for entry in selected)
        (BASE / filename).write_text(yaml.safe_dump(selected, allow_unicode=True, sort_keys=False), encoding="utf-8")
